...
const report = {
  schemaVersion: 1,
  model: 'distributed-pressure-v2-core-transition-chain-profile-v0.1',
...
if (json) {
  ...
  console.log(`transition=${chain.transition?.status} bridgeUpper=${chain.bridge?.optimisticTerminalHpUpperBound ?? null}`);
