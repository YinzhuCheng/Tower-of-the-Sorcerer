import { analyzeThresholdCoreEconomyBridgeWave } from '../src/analyzer/event-order-core-economy-bridge-wave.js';
...
const analysis = analyzeThresholdCoreEconomyBridgeWave({
...
const report = {
  schemaVersion: 1,
  model: 'distributed-pressure-v3-economy-bridge-wave-v0.1',
  rebuild: {
...
if (json) {
  ...
  console.log(`V3 economy bridge wave status=${analysis.status} exploit=${analysis.exploitFound}`);
  console.log(`reference=${rebuilt.terminalHp} semantic=${rebuilt.semanticFingerprint}`);
  for (const attempt of analysis.prefixAttempts ?? []) {
    console.log(`prefix=${attempt.prefixCertificateHash} purchaseHistogram=${JSON.stringify(attempt.purchaseHistogram)} selected=${JSON.stringify(attempt.selectedBridge)}`);
  }
  for (const attempt of analysis.suffix?.attempts ?? []) {
    const s = attempt.solver ?? {};
    const t = s.telemetry ?? {};
    console.log(`suffix prefix=${attempt.prefixCertificateHash} bridge=${attempt.bridgeId} hp=${attempt.resources?.hp} gold=${attempt.resources?.gold} p=${attempt.shopPurchases} ub=${attempt.upperBound} expanded=${s.expandedStates} generated=${s.generatedStates} bound=${s.prunedBound} exact=${s.exact} stop=${s.stoppedReason} travel=${t.travelGeneratedRatio} late=${t.lateFloorExpandedRatio} exploit=${attempt.exploit}`);
  }
