...
and appeared balanced under greedy event order + purchase 1-opt (7,083 HP, 11.62% minimum margin). Event-order search then found a 7,187 route, and purchase 1-opt on that event skeleton reached **7,687 HP with a 42.63% minimum margin**.

Therefore V1 is too forgiving under the stronger player model.
...
No best-response monotonicity violation was observed on these samples.

The target center was 16.5%, so the current preferred scale is **0.8375**.
...
The V2 reference witness at this scale has:
...
The purchase policy is stored in `src/tuner/review-candidates.js`. The 241-step topology/action witness is **not** hard-coded there; `review-candidate-v2-rebuild.js` regenerates it from repository algorithms.

## Reference trust model

V2 introduces an explicit reference mode:

```text
referenceMode = event-order-step-witness
```

A stored HP/hash is insufficient. Before 4,578 can be used as a threshold, the validator must:

1. rebuild the witness from repository algorithms;
2. match its witness hash and purchase sequence to the snapshot;
3. apply the V2 numeric overlay;
4. replay every action through canonical `engine.js`;
5. verify terminal HP and minimum margin.

Only then may an event-order threshold proof ask whether a stronger route exists.

This is implemented in `src/tuner/review-candidate-reference.js`.

## Validation stack

`review-candidate-v2-validation.js` runs four independent layers:

1. **reference reconstruction/replay** — validates the 4,578 witness;
