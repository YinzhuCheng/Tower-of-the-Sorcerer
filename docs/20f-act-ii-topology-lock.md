...
- F10 的 `voidCore` 完整击败后不再结算胜利：它恢复主角 **100 / 100 MP**，解锁
  `魔力附刃`，并解除通往 F11 的楼梯。
...
1. 为 F10–F20 写 Act II content overlay：F10 觉醒、F11–F20 地图占位、敌人/物品 ID。
2. 房间化：每层放置入口、枢纽、侧室、前庭、Boss、楼梯与门，并以静态可达性确保
   所有结界都是割点，门后确有对应权限或收益。
3. 冻结完整地图、Boss/宝物/门/楼梯坐标及卡片状态图，生成 20 层截图。
4. 修复并扩展求解器的状态压缩与 MP 动作枚举，保存可回放路线证书。
