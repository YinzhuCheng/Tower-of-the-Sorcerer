...
/**
 * Counterexample-first continuation of the c6 zero-damage closure bridge screen.
 * The closure exposed much stronger p21/p22 c7 states, so this analyzer spends
 * terminal search only on a six-family residual portfolio and requires an
 * ordinary fixed-adapter replay before accepting any exploit.
 */
export function analyzeV3C6ZeroDamageClosureCounterexampleHunt({
  ...
  return withBalanceEdits(snapshot.edits, () => {
    ...
    if (!reference.ok || !Number.isFinite(reference.terminalHp)) {
      return {
        schemaVersion: 1,
        model: 'v3-c6-zero-damage-closure-counterexample-hunt-v0.1',
    ...
    const fromFrontier = collectGoalFrontier({
      ...
      solverVersion: `v3-closure-hunt-core${fromCores}-prefix-v0.1`
    });
    ...
    for (const prefix of scheduledPrefixes) {
      ...
      const bridgeFrontier = collectGoalFrontier({
        ...
        solverVersion: `v3-closure-hunt-core${fromCores}-to-core${toCores}-v0.1-g${bridgeMaxGoalsPerPrefix}`
      });
      let replayable = 0;
      let relevant = 0;
    ...
    for (const bridge of scheduledResidualBridges) {
      const solver = solve({
        ...
        solverVersion: `v3-closure-hunt-core${toCores}-suffix-v0.1-b${suffixPrioritySlackBucket}`
      });
    ...
    return {
      ...
      interpretation: exploit
        ? (exploit.witnessReplay.ok && exploit.witnessReplay.objective === exploit.terminalHp
            ? 'closure_exposed_a_stronger_fixed_policy_event_order_route_and_the_combined_witness_replayed_on_the_ordinary_authoritative_adapter'
            : 'closure_suffix_found_a_threshold_exploit_but_ordinary_combined_witness_replay_needs_attention')
        : 'closure_exposed_higher_resource_c7_residuals_but_the_sampled_terminal_suffix_portfolio_did_not_find_a_replayable_exploit'
    };
  });
}
