...
/**
 * Counterexample-first staged search using the Compass-assisted cross-floor
 * monotone closure. Any threshold crossing is accepted only after the combined
 * witness replays on the ordinary fixed-purchase adapter / authoritative engine.
 */
export function analyzeV3C6CrossFloorZeroDamageCounterexampleHunt({
  ...
  return withBalanceEdits(snapshot.edits, () => {
    ...
    if (!reference.ok || !Number.isFinite(reference.terminalHp)) {
      return {
        schemaVersion: 1,
        model: 'v3-c6-cross-floor-zero-damage-counterexample-hunt-v0.1',
    ...
    const fromFrontier = collectGoalFrontier({
      ...
      solverVersion: `v3-cross-floor-hunt-core${fromCores}-prefix-v0.1`
    });
    ...
    for (const prefix of scheduledPrefixes) {
      ...
      const bridgeFrontier = collectGoalFrontier({
        ...
        solverVersion: `v3-cross-floor-hunt-core${fromCores}-to-core${toCores}-v0.1-g${bridgeMaxGoalsPerPrefix}`
      });
      let replayable = 0;
      let relevant = 0;
    ...
    const suffixThresholdAdapter = createObjectiveThresholdAdapter({ threshold, baseAdapter: orderedSuffixBaseAdapter });
    ...
    for (const bridge of scheduledResidualBridges) {
      const solver = solve({
        ...
        solverVersion: `v3-cross-floor-hunt-core${toCores}-suffix-v0.1-b${suffixPrioritySlackBucket}`
      });
      const suffixCertificate = solver.certificate;
      const replay = suffixCertificate
        ? replayTowerCertificate(suffixCertificate, { adapter: suffixThresholdAdapter, initialState: bridge.state })
        : null;
    ...
    return {
      ...
      interpretation: exploit
        ? (exploit.witnessReplay.ok && exploit.witnessReplay.objective === exploit.terminalHp
            ? 'Compass cross-floor closure exposed a stronger legal V3 route and the combined witness replayed on the ordinary authoritative fixed-purchase adapter'
            : 'cross-floor closure suffix found a threshold exploit but ordinary combined witness replay needs attention')
        : 'Compass cross-floor normalization reduced harvest ordering but the sampled residual suffix portfolio still did not find a replayable V3 exploit'
    };
  });
}
