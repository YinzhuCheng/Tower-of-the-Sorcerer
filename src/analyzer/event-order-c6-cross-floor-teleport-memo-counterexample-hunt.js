...
import { createFixedPurchaseTeleportTransitionMemoAdapter } from '../solver/fixed-purchase-teleport-transition-memo-adapter.js';
...
function compactSolver(report, memoStats = null) {
  return {
...
/**
 * A/B counterexample search that keeps the existing c6 -> c7 bridge construction
 * and ordinary Compass hub semantics, but de-duplicates only provably identical
 * teleport transitions within each independent c7 suffix solve.
 */
export function analyzeV3C6CrossFloorTeleportMemoCounterexampleHunt({
  ...
  return withBalanceEdits(snapshot.edits, () => {
    ...
    if (!reference.ok || !Number.isFinite(reference.terminalHp)) {
      return {
        schemaVersion: 1,
        model: 'v3-c6-cross-floor-teleport-memo-counterexample-hunt-v0.1',
    ...
    const fromFrontier = collectGoalFrontier({
      ...
      solverVersion: `v3-teleport-memo-core${fromCores}-prefix-v0.1`
    });
    ...
    for (const prefix of scheduledPrefixes) {
      ...
      const bridgeFrontier = collectGoalFrontier({
        ...
        solverVersion: `v3-teleport-memo-core${fromCores}-to-core${toCores}-v0.1-g${bridgeMaxGoalsPerPrefix}`
      });
      let replayable = 0;
      let relevant = 0;
    ...
    for (const bridge of scheduledResidualBridges) {
      // IMPORTANT: one fresh memo instance per independent solve. Sharing this
      // adapter across bridge starts would make reachability provenance invalid.
      const memoSuffixBaseAdapter = createFixedPurchaseTeleportTransitionMemoAdapter({
        baseAdapter: searchAdapter,
        minCores: toCores
      });
      const orderedSuffixBaseAdapter = createLateGameThresholdPriorityAdapter({
        baseAdapter: memoSuffixBaseAdapter,
        threshold,
        minCores: toCores,
      ...
      const solver = solve({
        ...
        solverVersion: `v3-teleport-memo-core${toCores}-suffix-v0.1-b${suffixPrioritySlackBucket}`
      });
      const memoStats = memoSuffixBaseAdapter.teleportTransitionMemoStats();
      ...
      suffixAttempts.push({
        ...
        solver: compactSolver(solver, memoStats),
        replay: replay ? {
          ok: replay.ok,
    ...
    return {
      ...
      suffix: {
        actionModel: 'ordinary-compass-hubs+solve-scoped-equivalent-teleport-memo-v1',
      ...
      interpretation: exploit
        ? (exploit.witnessReplay.ok && exploit.witnessReplay.objective === exploit.terminalHp
            ? 'Solve-scoped equivalent-teleport memo exposed a stronger legal V3 route and the ordinary certificate replayed on the authoritative fixed-purchase adapter'
            : 'teleport-memo suffix found a threshold exploit but ordinary combined witness replay needs attention')
        : 'Equivalent Compass teleport transitions were de-duplicated without changing hub semantics, but the sampled residual suffix portfolio still did not find a replayable V3 exploit'
    };
  });
}
