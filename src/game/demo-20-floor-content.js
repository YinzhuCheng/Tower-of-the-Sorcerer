...
function installActTwoDialogues(dialogues) {
  Object.assign(dialogues, {
    ...
    floor13: dialogueSequence('第十三阵：脉冲锻炉', [
      ...
      dialogueTurn('绫星·璃', 'hero', '现场值守人做不到同时改时限、改模式、禁用停机。这段写入绕过了女王的王庭，来自起源魔源的主权权限。这枚主权印留下了清楚的来路，不是我们的推测。', { expression: 'resolve' }),
    ...
    floor15: dialogueSequence('第十五阵：折页档案馆', [
      ...
      dialogueTurn('残响精灵·纱雾', 'guide', '停电账补上了缺的那一块。主权签名出现十七分钟后，主线断了。三席那时只能看见各自终端弹出的现场问题，读不到主权者先前写下的“无限延长”。这就解释了为什么同一个“是”，可能回答的是完全不同的问题。', { expression: 'focus' }),
    ...
    floor17: dialogueSequence('第十七阵：三冠阶庭', [
      ...
      dialogueTurn('无声女王·诺克缇娅', 'final_queen', '我认得名簿席那次回答，因为当时就是我守在王庭的终端前。灯火全灭，下面的人还在报名字，我只想让那块屏幕再亮一会儿，等他们把最后几个人说完。', { expression: 'sorrow' }),
    ...
    floor18: dialogueSequence('第十八阵：澄空航渠', [
      ...
      dialogueTurn('无声女王·诺克缇娅', 'final_queen', '我在王庭等不到回执，以为是风暴吞掉了它。所以每次终端再问“是否继续等待”，我都选了“是”。原来回执早已进塔，只是被藏在我看不见的权限层里。', { expression: 'sorrow' }),
    ...
    bossArcaneSovereignPost: dialogueSequence('主权封印解除', [
      ...
      dialogueTurn('绫星·璃', 'hero', '第二阶段要停下的是强制命令，不是记录库。打开写入口。', { expression: 'resolve' })
    ]),
