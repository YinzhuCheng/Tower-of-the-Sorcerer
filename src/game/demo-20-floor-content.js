...
function installActTwoDialogues(dialogues) {
  Object.assign(dialogues, {
    ...
    floor14: dialogueSequence('第十四阵：三矢竞技场', [
      dialogueTurn('旁白', null, '锻炉铜带上的去向签将一行人引到三矢竞技场。这里并非为观众表演武技，而是在断讯时仍能保住原始回答的核验场。即使高塔主线后来被改写，航路、补给、名簿三席当时的答复也会分开留存。', { kind: 'narration' }),
    ...
    floor18: dialogueSequence('第十八阵：澄空航渠', [
      ...
      dialogueTurn('影织姬·鸦羽', 'shadow_boss', '找到了。不是人在偷信，是这东西。断联时它会接过航渠，只认上层那句：“全员安全前，任何结案回执均不可信。”它不读信，只认印权。三年来，一直如此。'),
      ...
      dialogueTurn('影织姬·鸦羽', 'shadow_boss', '可离港回执只能证明第七船已经开走，不能证明灰港每一个名字的最后去向。如果我们拿着一张船单就说“所有人都没事”，我们也是在把看不见的人从纸上擦掉。'),
    ...
    floor19: dialogueSequence('第十九阵：回响王庭', [
      dialogueTurn('旁白', null, '虚空先驱倒下后，鸦羽从它胸口取出原始离港回执。船长印、航站时刻、四十七名乘员中那二十四人的配药记录，与澜音记得的最后一句报告全部相符：第七船确已离港。', { kind: 'narration' }),
      dialogueTurn('旁白', null, '回响王庭中没有王座，只有数百块悬在月光里的玻璃名牌。有的写着船号，有的系着黑带，还有的只留下“最后见于北码头，待家属确认”。', { kind: 'narration' }),
      dialogueTurn('回声摄政官', 'echo_regent', '停在门外。先分清一件事：北辰七号那四十七人已经由回执证明全部到岸；这本册子记的却是整场灰港灾难里所有登记过的人，不只最后一船。人们叫它“死亡名簿”，其实并不只记死者：离港者记航次和抵达地，罹难者记发现地与确认人，暂时失联者则保留最后线索与复查日期。', { expression: 'grave' }),
      ...
      dialogueTurn('残响精灵·纱雾', 'guide', '现在回执能为第七船的登船者补上离港记载，死者页保留已核实的罹难者，待核实页也继续保留最后线索。结案不是宣布“所有人都平安”，而是不再用一句模糊的“等待确认”替所有人作答。', { expression: 'focus' }),
      dialogueTurn('绫星·璃', 'hero', '我们要带上的不是整本原件，而是可以验证的副本：每个人当前的判断、证据来源和最后一次改动时间。原件留在你这里，起源核心不能拿它继续驱使活人，我们也不会将它当成战利品。', { expression: 'resolve' }),
