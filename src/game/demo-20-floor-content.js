...
function installActTwoDialogues(dialogues) {
  Object.assign(dialogues, {
    ...
    floor13: dialogueSequence('第十三阵：脉冲锻炉', [
      ...
      dialogueTurn('龙姬·焰璃', 'dragon_boss', '这三天不是判断人该不该被忘记的期限，而是供能部门的安全约定：三日后必须停止满载，先检修炉管，再转为只留档案、维持微光的低耗状态。可我等了三年，计时从没到过终点。'),
      ...
      dialogueTurn('残响精灵·纱雾', 'guide', '铜带里的修改不只删掉了三日时限。它还把“归档待机”改成“持续救援”，封住现场停机键，甚至设定“未确认全员安全前自行重新启动”。', { expression: 'focus' }),
      dialogueTurn('绫星·璃', 'hero', '现场值守人做不到同时改时限、改运转方式、封死停炉手段。这段写入绕过了女王的王庭，来自起源魔源的主权权限。这枚主权印留下了清楚的来路，不是我们的推测。', { expression: 'resolve' }),
      ...
      dialogueTurn('龙姬·焰璃', 'dragon_boss', '改令记录随后被送往三矢竞技场，由航路、补给、名簿三席的代理守卫校验。那三枚印一定记得谁先写入，谁后回答。'),
    ...
    floor14: dialogueSequence('第十四阵：三矢竞技场', [
      dialogueTurn('旁白', null, '锻炉铜带上的去向签将一行人引到三矢竞技场。这里并非为观众表演武技，而是紧急命令的断讯时仍能保留原始回答的核验场：即使高塔主线被人改写，原始回答仍会被拆成三份保存。', { kind: 'narration' }),
    ...
    floor15: dialogueSequence('第十五阵：折页档案馆', [
      ...
      dialogueTurn('旁白', null, '珂珂从柜台夹层抽出一张磨起毛边的单据。船号、五只药箱、二十四人、船长签名和交接时刻一应俱全；唯独高塔总账的“结清”栏始终空白。', { kind: 'narration' }),
    ...
    floor16: dialogueSequence('第十六阵：镜轮双殿', [
      dialogueTurn('旁白', null, '珂珂的交货原单被放进镜轮双殿中央。左镜照出说话者此刻的容貌与动作，右镜同时留下她的声音、时间和眼前的证据。两面镜子都不从旧名册自动填名，也不接受代签。', { kind: 'narration' }),
      dialogueTurn('深蓝歌姬·澜音', 'whale_boss', '我们已经有当年的物证，但起源核心还要求活着的人亲口说明“今天要怎样收尾”。这份新见证不会改写三年前的原件，它只会在原件后面增加新状态：停止强制执行，保留所有记录。'),
      dialogueTurn('深蓝歌姬·澜音', 'whale_boss', '米露看过补给怎样被重复发放，我听到了最后航船的回答，焰璃保存着供能曲线，鸦羽则能追踪被藏进上层封印的线路。四个人都有自己见过的部分，没有人能一个人证明全部。'),
      ...
      dialogueTurn('深蓝歌姬·澜音', 'whale_boss', '我们正在修复的，就是一个人用一枚总印替三个部门作答的错误。如果新的结案仍由发令者替每个人一起作答，我们只是用另一道命令盖住了旧错。'),
    ...
    floor18: dialogueSequence('第十八阵：澄空航渠', [
      ...
      dialogueTurn('影织姬·鸦羽', 'shadow_boss', '别踩那条最亮的绿线。它看起来直通总账，实际上在第三个弯口潜入主权封印。每一封离港回执进去以后，都会被改标为“待全员安全后复核”。'),
      dialogueTurn('绫星·璃', 'hero', '又是“全员安全前”。锻炉靠这句话不停重启，这条航渠则靠它把回执拐走。主权者删掉的不只是截止期，他还顺手把“什么算结束”写成了一个永远等不到的答案。'),
      ...
      dialogueTurn('影织姬·鸦羽', 'shadow_boss', '找到了。不是人在偷信，是这东西。断联时它会自动接管航渠，只认上层那句：“全员安全前，任何结案回执均不可信。”它不读信，只看权限。三年来，一直如此。'),
      dialogueTurn('无声女王·诺克缇娅', 'final_queen', '我在王庭等不到回执，以为是风暴吞掉了它。所以每次问询镜再亮起“是否继续等待”，我都选了“是”。原来回执早已进塔，只是被藏在我看不见的权限层里。', { expression: 'sorrow' }),
      ...
      dialogueTurn('绫星·璃', 'hero', '先把回执拿回来。然后去回响王庭。船单只能告诉我们谁离港，不能替死者、失联者和还没查清的人说话。下一步，把每个名字放回她真正的位置。', { expression: 'resolve' })
    ]),
    ...
    floor20: dialogueSequence('第二十阵：起源魔源', [
      ...
      dialogueTurn('旁白', null, '绫星·璃没有立即拔剑。她先将四份证据依次放在誊录台上：三席的原始时序、珂珂的交付账、第七船的离港回执、死亡名簿的分类校验副本。它们分别回答“谁同意”、“物资交给了谁”、“船是否离港”与“每个人最后处于什么状态”。', { kind: 'narration' }),
      ...
      dialogueTurn('奥术主权者', 'arcane_sovereign', '我以为只是多等几天。可回执一到，核心就把它视为与最高命令冲突，截进上层封印。后来我再想改，它已经把我的封印也锁进了命令里。', { expression: 'regret' }),
    ...
    warCouncil: dialogueSequence('王座前：共鸣会战', [
      ...
      dialogueTurn('影织姬·鸦羽', 'shadow_boss', '前庭共鸣池里还有一百二十点独立储量，每二十点算一档，一人最多接六十点。它和璃平时附刃用的魔力不是同一条回路；我们可以先在阵列里反复试配，不会因此消耗真正上路时的资源。'),
