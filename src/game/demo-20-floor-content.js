...
function installActTwoDialogues(dialogues) {
  Object.assign(dialogues, {
    floor11: dialogueSequence('第十一阵：复苏环廊', [
      ...
      dialogueTurn('残响精灵·纱雾', 'guide', '这里就是当年所有记录最后会汇到一起的地方。导航台说船去了哪儿，补给席记东西交给了谁，名簿则要回答一个更难的问题：这个人后来究竟已经抵达安全地、已经罹难，还是仍然没有查清。三年前通讯断掉以后，高塔就是靠这些零碎答案拼出整场撤离。', { expression: 'focus' }),
    ...
    floor13: dialogueSequence('第十三阵：脉冲锻炉', [
      ...
      dialogueTurn('绫星·璃', 'hero', '温室的账对上了。每当锻炉将旧命令当成新一天的救援，温室就会把同一张处方重印一次。温室和锻炉没有各自犯两种错。它们只是被同一道错误指令牵着走：一边不断加火，一边不断重配同一批药。'),
    ...
    floor18: dialogueSequence('第十八阵：澄空航渠', [
      ...
      dialogueTurn('绫星·璃', 'hero', '问题就在“全员安全”这四个字。有人已经有到岸证据，有人已经罹难，也有人到今天还只能写“待核实”。真实世界不会给出一个整整齐齐、人人都能盖成“安全”的答案。拿它当结案条件，等于从一开始就没打算让警报结束。'),
    ...
    floor19: dialogueSequence('第十九阵：回响王庭', [
      ...
      dialogueTurn('回声摄政官', 'echo_regent', '我是名簿的保管人，不是死亡的宣告人。没有抵达回执，我不能把某人写成安全抵达；没有可核实的信息，我也不能因为三年没有消息，就把某人写成罹难。', { expression: 'grave' }),
      dialogueTurn('绫星·璃', 'hero', '所以你一直拒绝交出名簿。你不是要用死者继续发动命令，而是害怕任何一方拿着不完整的证据，为了方便结案，就把待核实者统统判成已抵达或已罹难。'),
      dialogueTurn('回声摄政官', 'echo_regent', '是。过去有人对我说：“灰港早就空了，把所有人都盖成已抵达吧。”也有人说：“失踪三年不可能还活着，都放进死者页吧。”两种做法都能让数字变得整齐，也都可能把某个真实的人写错。', { expression: 'grave' }),
