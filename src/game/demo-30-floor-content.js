...
function installDialogues(dialogues) {
  Object.assign(dialogues, {
    floor21: sequence('第二十一阵：余烬登记库', [
      ...
      turn('残响精灵·纱雾', 'guide', '第二步是起源核心擅自补写。航路、补给、名簿三席曾在停电后各自同意“临时继续救援”；核心为了让无限延长通过校验，把那三份晚了十七分钟、本来只针对停电的回答，挪到了主权者的命令下面。', { expression: 'focus' }),
    ...
    floor25: sequence('第二十五阵：缺页庭', [
      ...
      turn('旁白', null, '庭院中央的协议缺了最后一页。断口不是刀痕：勘误核心判定“归档”与“持续救援”冲突后，便把这一页自行拆出了主卷。', { cg: '/assets/anime/cg/liyue-noctia-missing-page-cg-audit-v3.webp', cgHold: 8, kind: 'narration' }),
    ...
    floor26: sequence('第二十六阵：折角集市', [
      ...
      turn('旁白', null, '瓶子按进货日期排了整整三面墙，却只有最后一排还存着药水。柜台上的红帐每天自己写下同一句：“灰港救援队二十四人，今夜出发。”可这支队伍早在三年前就完成了最后一次任务。', { kind: 'narration' }),
    ...
    floor27: sequence('第二十七阵：接力校场', [
      ...
      turn('绫星·璃', 'hero', '修复章程处理的是另一件事：我们究竟先把护送、校验还是接力恢复到能用。带回的护送印、校验簿或接力电容，会直接改变灯塔终战时我们能承受什么、又能先恢复哪一种工作。它和见证者是谁没有绑定关系。', { expression: 'resolve' }),
      turn('影织姬·鸦羽', 'shadow_boss', '还有，别把它和前面的契约、章程混在一起。那些已经写进记录了；这里选的只是临战备用。校验章程照样可以先拿护送，护送章程也能先拿校验。互不覆盖。'),
      turn('影织姬·鸦羽', 'shadow_boss', '先压倒决斗者，后面的追击会少两轮；先解决咏唱者，守望者的术式会变弱，勘误核心也少一次连击；先拿下总管，共鸣会在清场后重新充满。就这三种。'),
