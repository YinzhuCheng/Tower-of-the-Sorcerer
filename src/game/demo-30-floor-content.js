...
function installDialogues(dialogues) {
  Object.assign(dialogues, {
    floor21: sequence('第二十一阵：余烬登记库', [
      ...
      turn('残响精灵·纱雾', 'guide', '三件事终于接上了：主权者删掉三日期限，起源核心把三席旧回答接错了地方，虚空先驱又截走抵达回执。灰港早就可以停止救援，只是该抵达同一张桌面的证据，被拆散在不同地方。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '第一步发生在风暴断讯时。奥术主权者怕三天后还有生还者没被找到，所以把“三日后复核撤销”改成“无限延长”。他想防止过早收队，却没写新的停止条件。', { expression: 'focus' }),
      turn('奥术主权者', 'arcane_sovereign', '我当时以为，多等一天不过是多烧几盏灯。可我没有站在灰港的雨里，没有看见米露的爪子、焰璃的炉火、那些每天被重新扣掉的药和守卫的三年。代价落在离我太远的人身上，所以我把自己的担心写成了一道别人无法退出的命令。', { expression: 'regret' }),
      ...
      turn('无声女王·诺克缇娅', 'final_queen', '我封塔，是因为我以为警报一停，这些名字就会被当成从未存在。我现在才看见：有人已经抵达安全地，有人已经罹难，也有人仍需查找。被我压成一句“等待确认”的，恰恰是这些彼此不同的答案。', { expression: 'sorrow' }),
      ...
      turn('残响精灵·纱雾', 'guide', '墙上三块灯牌不是三种互相冲突的答案。我们已经知道发生了什么；现在要选的只是先补哪处裂口——先护住回执，先把真假分清，还是先让结案真正送到外面的哨站。', { expression: 'gentle' }),
      ...
      turn('绫星·璃', 'hero', '签下章程本身不会立刻拿走任何东西。代价都写在对应侧库门前；另外两座侧库也不会因此消失。先看手里的卡牌和还剩多少余地，再决定把哪一段修复放在最前面。', { expression: 'resolve' })
    ]),
    ...
    floor23: sequence('第二十三阵：逐页校验室', [
      ...
      turn('绫星·璃', 'hero', '别再让“哪页更新”替我们决定真假。姓名、船号、时刻、来源、经手人、传递路径，一项项对；能对上的先认，对不上的就把冲突写出来，别因为一处空白把整页扔掉。', { expression: 'resolve' }),
    ...
    floor24: sequence('第二十四阵：灯塔接力室', [
      ...
      turn('龙姬·焰璃', 'dragon_boss', '灰港早已有足够证据结束救援，只是灯塔一直没把“救援已结束”的结论送过最后三座哨站。这里要修的不是证据本身，而是外面的人能不能真正收到这句话。'),
    ...
    floor25: sequence('第二十五阵：缺页庭', [
      ...
      turn('奥术主权者', 'arcane_sovereign', '是我删掉三日期限，也是我的命令触发了这次拆页。我当时只想着“先别太早收队”，于是亲手写下了“无限延长”，却没有写新的停止条件。我以为之后还能回来撤掉它；核心却只会照字面执行。这个口子，是我留下的。', { expression: 'regret' }),
    ...
    floor27: sequence('第二十七阵：接力校场', [
      ...
      turn('绫星·璃', 'hero', '明白。见证契约决定的是谁能亲手留下自己的那一页，不决定灰港有没有资格结案。谁没走完，也不会让已经查清的事实失效；谁走完了，我们就把纸和笔交回她自己手里。', { expression: 'resolve' })
    ]),
    ...
    floor29: sequence('第二十九阵：最后索引', [
      ...
      turn('残响精灵·纱雾', 'guide', '新索引只写四种现况：已抵达、已罹难、待核实、待投递。它们不是给人分等级，只是告诉下一个接手的人：我们现在知道什么，还欠哪一步。新证据可以改判断，但旧的判断和改动理由必须留下。', { expression: 'focus' }),
    ...
    floor30: sequence('第三十阵：余烬灯塔', [
      turn('旁白', null, '最后保管人的封底钥匙转动时，整座索引室向两侧打开。新索引、缺失的归档页、抵达回执和死亡名簿被同一道升降轨送向灯塔。璃走在最前，奥术主权者与诺克缇娅亲自抬着最重的原卷。', { cg: '/assets/anime/cg/liyue-archive-warden-entry-cg-audit-v3.webp', cgHold: 5, kind: 'narration' }),
      ...
      turn('奥术主权者', 'arcane_sovereign', '别再往黑暗里找第四个人了。三日期限是我删的；三席那枚“同意”是核心拿旧回答补出来的；那份抵达回执，则被虚空先驱按我留下的错误命令截进了上层封印。走到这里，我们已经把每一只手都看见了。', { expression: 'regret' }),
      ...
      turn('绫星·璃', 'hero', '不用再找第四个人了。该负责的人、写错的位置、截走回执的东西，我们都已经看见。现在要做的是把这些环一处处拆开，别让同样的错误再接成一条锁链。', { expression: 'resolve' }),
      turn('无声女王·诺克缇娅', 'final_queen', '我们带来了那份回执、死亡名簿和被拆走的归档条款。它们证明灰港可以结案，也证明停止警报不会删除任何一个名字。', { expression: 'grave' }),
      turn('无声女王·诺克缇娅', 'final_queen', '这三份东西谁也替不了谁。抵达回执告诉我们北辰七号确实到了北岸，却不能替其他船次的罹难者和待核实者作答；名簿把罹难者和待核实者分开，却不能证明船真的到岸；归档条款则告诉我们，警报停下以后原件照样能留下。三份叠在一起，我们才不是靠一句“大家都没事了”草草收尾。', { expression: 'grave' }),
      turn('残响精灵·纱雾', 'guide', '交接分四步：封存旧原件，另添现况与经手注记，由原签署人撤销无限延长，再由当年亲手执行封塔的人见证这次结案送进灯塔。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '先把灰港原卷封存起来，谁也不能改动原文。求援、抵达回执、名簿，还有我们一路查到的那些错误印记，全都原样留下。谁都不能为了让今天看起来更体面，就回头把三年前改成一份漂亮的故事。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '然后在原卷旁另添两类注记：一类只写现在知道的情况——已抵达、已罹难、待核实、待投递；另一类写明是谁根据什么证据改了判断、何时改、为什么改。以后有新证据，只改这些注记，不碰旧原文。', { expression: 'focus' }),
      ...
      turn('无声女王·诺克缇娅', 'final_queen', '最后一步我来见证。我是当年亲手执行封塔的人，不能到了结案时又躲到名簿后面。哪些人已有证据确认抵达安全地，哪些人已罹难，哪些名字还要继续查，我会一项项确认；结案送进灯塔以后，七天后的第一次复查，我也会回来。', { expression: 'grave' }),
      turn('绫星·璃', 'hero', '四步都做完，登记网才会真正停止发令，只保留档案。现在若直接熄塔，复明后旧抄本还会把救援重新叫醒；那些没人标出去向的信，也会再次变成孤立原件。', { expression: 'resolve' }),
      turn('绫星·璃', 'hero', '我们之前做过的选择，都会跟着进灯塔：章程决定哪一处修复先替我们分担压力，校场选定的备用术式则在交战时帮忙。一路受过的伤、剩下的魔力和手里还没用掉的卡牌都会原样带进去——这扇门不会替我们把代价抹掉。', { expression: 'resolve' }),
      turn('残响精灵·纱雾', 'guide', '登记库里真正修好的章程，到了灯塔仍然有效；校场带来的备用术式则另算一层支援。两份准备不必来自同一条路。开战前再确认一次，别把一路做过的准备忘在门外。', { expression: 'focus' }),
      turn('旁白', null, '璃将抵达回执收进胸前的硬皮夹，又用两条封带把死亡名簿固定在背后。她知道守望者不是最后的幕后真凶，也没有期待几句说服就能让它违背保护原件的职责。这一战要回答的问题很具体：她能不能在这柄长枪面前，不丢掉任何一页地走到誊录台。', { kind: 'narration' }),
    ...
    bossArchiveWardenPost: sequence('灯塔：守望者收枪', [
      ...
      turn('残响精灵·纱雾', 'guide', '新的现况注记开始逐名分流：有离港与到岸双重记录的，标为“已抵达”；死亡名簿中已确认身份的，标为“已罹难”；两份名单仍有冲突的，标为“待核实，七日后复查”；家书则按收件地进入“待投递”。', { expression: 'focus' }),
      turn('奥术主权者', 'arcane_sovereign', '我用原签名撤销无限延长，只撤强制命令，不撤档案。今后的新求援另编新号，不能再冒充灰港旧案。', { expression: 'acceptance' }),
      turn('旁白', null, '奥术主权者将手按在自己三年前的签名上。光线没有擦除那道笔迹，而是在旁边生成一条新记录：“撤销无限延长；理由：抵达回执已找回，三席确认时序错置，归档条款已恢复；签署人：奥术主权者。”', { kind: 'narration' }),
    ...
    ending: sequence('终章：未投递的信', [
      ...
      turn('无声女王·诺克缇娅', 'final_queen', '已确认抵达的人写明航次与目的地，罹难者回到名簿，仍待联系的人保留复查日期。没有谁再被一句“等待确认”困在同一夜。', { expression: 'knowing' }),
      turn('无声女王·诺克缇娅', 'final_queen', '人到了岸，也还要确认她后来有没有住处；人已经罹难，也还要把消息和遗物送到家人手里；没查清的名字，就按复查日继续找。我们今天结束的是那道无限命令，不是以后还要做的这些事。', { expression: 'knowing' }),
      ...
      turn('残响精灵·纱雾', 'guide', '灰港不会因为我们先修了哪一处，就得到不同的结案。区别只在灯塔恢复日常以后，大家先把哪件事继续做下去：护送、校验，还是接力。', { expression: 'gentle' }),
      ...
      turn('绫星·璃', 'hero', '结案已经完整。该停的警报停了，该留的名字留下了，没查清的人也有人继续负责。至于谁还想在共同记录后写一页自己的话，就让她自己写——我们已经替别人决定了太久，不必再替任何人把余生写成同一种结论。', { expression: 'resolve' }),
