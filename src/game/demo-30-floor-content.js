...
function installDialogues(dialogues) {
  Object.assign(dialogues, {
    floor21: sequence('第二十一阵：余烬登记库', [
      ...
      turn('残响精灵·纱雾', 'guide', '第一步发生在风暴断讯时。奥术主权者怕三天后还有生还者没被找到，所以把“三日后复核撤销”改成“无限延长”。她想防止过早收队，却没写新的停止条件。', { expression: 'focus' }),
    ...
    floor25: sequence('第二十五阵：缺页庭', [
      ...
      turn('旁白', null, '庭院中央的协议缺了最后一页。断口不是刀痕：勘误核心判定“归档”与“持续救援”冲突后，自动把这一页拆出了主卷。', { kind: 'narration' }),
      ...
      turn('无声女王·诺克缇娅', 'final_queen', '诺克缇娅蹲下，将散落的页角一片片拼回断口。她终于明白，结束救援和忘记灰港从来不是同一件事。', { expression: 'grave' }),
    ...
    floor28: sequence('第二十八阵：归档风暴', [
      ...
      turn('无声女王·诺克缇娅', 'final_queen', '这一次，诺克缇娅没有追着那张纸走进风里。她解下披风，和璃一起把近处的信挡在墙角，又用封条将它们按原有编号排好。抓住一封信不能让它完成投递，但至少能先防止它又被撕成新的警报。', { expression: 'grave' }),
    ...
    floor29: sequence('第二十九阵：最后索引', [
      ...
      turn('最后保管人', 'crown_magus', '我知道许多人已经离港，却不能在空白处写“待补”。旧规则逼我在伪造完整与抹掉整页之间选一个。'),
      turn('最后保管人', 'crown_magus', '我试过把不清楚的部分空着。索引兽会叼走整页，把它重新发给已经不存在的救援队。我也试过填上猜测的日期，那页虽然通过了，我却再也无法分辨上面哪一部分是证据，哪一部分是我为了让它通过而填的话。'),
      turn('奥术主权者', 'arcane_sovereign', '这是我留下的设计。我怕一次遗漏害死人，便不允许记录带着空白结案；结果所有不完整记录都成了永远不能结束的警报。', { expression: 'regret' }),
      turn('奥术主权者', 'arcane_sovereign', '我把空白只当成玩忽职守，没有给“目前真的不知道”留位置。所以保管人想诚实，就必须让整页作废；想保住已知的部分，又只能用猜测补满。这不是他一个人的过错，是我把错误的两选一写进了他的岗位。', { expression: 'regret' }),
      ...
      turn('最后保管人', 'crown_magus', '保管人终于松开封底钥匙。击败左右索引守卫，我便把它交给你们——请让后来的人能承认“不知道”，也仍有办法继续查。'),
    ...
    floor30: sequence('第三十阵：余烬灯塔', [
      turn('旁白', null, '最后保管人的封底钥匙转动时，整座索引室向两侧打开。新索引、缺失的归档页、离港回执和死亡名簿被同一道升降轨送向灯塔。璃走在最前，奥术主权者与诺克缇娅亲自抬着最重的原卷。', { kind: 'narration' }),
    ...
    bossArchiveWardenPost: sequence('灯塔：守望者停机', [
      ...
      turn('残响精灵·纱雾', 'guide', '纱雾把光笔插进写入口：记录编号、来源、当前状态、经手人、修改时间、修改理由。旧版本全部转为只读，不再覆盖。', { expression: 'focus' }),
      ...
      turn('无声女王·诺克缇娅', 'final_queen', '诺克缇娅展开三位本人见证留下的镜印，又将自己的名字写在复查人一栏。她不再用沉默替灰港回答。', { expression: 'knowing' }),
    ...
    ending: sequence('终章：未投递的信', [
      turn('旁白', null, '勘误核心碎裂后，余烬灯塔没有熄灭。它换了一种平静的灯语：“已收到，编号灰港-终卷，值守人纱雾，七日后复查。”', { cg: '/assets/anime/cg/liyue-lighthouse-archive-cg.webp', kind: 'narration' }),
      ...
      turn('无声女王·诺克缇娅', 'final_queen', '她摘下王冠旁那枚从未熄灭的警报石，放在灰港终卷边。等名单整理完，我会亲自走出这座塔。', { expression: 'knowing' }),
      ...
      turn('旁白', null, '灯塔大门在清晨前打开。外面没有三年前的暴风，只有潮水退后湿润的石路，以及从新码头方向赶来的第一辆邮车。车夫没有带剑，他跳下车，展开一张空白签收表，问谁来交接第一批信。', { kind: 'narration' }),
