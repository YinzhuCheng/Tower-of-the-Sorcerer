...
function installDialogues(dialogues) {
  Object.assign(dialogues, {
    floor21: sequence('第二十一阵：余烬登记库', [
      ...
      turn('残响精灵·纱雾', 'guide', '夜航护送章程先解决“信会在路上被烧掉”。要打开那座侧库，需要两张月辉卡；守柜人会用实战确认我们能不能把原件完整护过警戒线。通过之后，护送印会加强我们的承伤能力，并在灯塔的两场硬仗里各替我们挡下三次反击。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '逐页校验先治的是另一种病：明明是真回执，却因为时间更早、格式不合，被当成假的。两张星蚀卡能打开校验内室。我们若在那里把姓名、船号、时间和来源一项项对齐，最后两具守卫依赖错误记录堆出来的强化也会一起松掉。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '灯塔接力解决的是最朴素、也最容易被忽略的问题：我们把结案写好了，外面到底有没有人收到。日曜、月辉各一张能把三座哨站重新接起来。它不会让最后的守卫凭空变弱，但会替我们把共鸣回路撑大，并留下两次真正能喘口气的充能机会。', { expression: 'focus' }),
      turn('绫星·璃', 'hero', '换句话说，三条路不是三个互相排斥的“正确答案”。护送让我们更能扛住最后的反击，校验让最后的敌人本身变弱，接力则把优势放在魔力储备和两次关键充能上。要选的是我们愿意先承担哪一种风险。', { expression: 'resolve' }),
    ...
    floor23: sequence('第二十三阵：逐页校验室', [
      ...
      turn('残响精灵·纱雾', 'guide', '如果我们签的是校验章程，两张星蚀卡可以开启内室；如果没有，就把星卡留给后面的路。拿到校验簿后，最后两具守卫依赖错误记录叠出来的增幅也会被一起削弱。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '这里真正要付的不只是两张卡。持簿执行官会用魔法反击直接削掉生命，所以得先确认我们吃得下这场代价。若能把校验簿带走，它会提前拆掉灯塔两具守卫大约五分之一的维持强度，并削弱它们的防护和魔法反击。', { expression: 'focus' }),
    ...
    floor24: sequence('第二十四阵：灯塔接力室', [
      ...
      turn('龙姬·焰璃', 'dragon_boss', '别再像三年前那样一着急就把功率推满。先把同一个结案编号送到第一站，等它真的回一句“收到”；再让第一站把它交给第二站，最后送到第三站和灰港新码头。哪一段断了，就只补那一段。我们不再因为一处没回话，就把整件事拖回风暴那一夜。'),
    ...
    floor27: sequence('第二十七阵：接力校场', [
      ...
      turn('影织姬·鸦羽', 'shadow_boss', '别找按钮。先击败谁，就是选择谁。核心一装进去，就不会自己跳到另一条跑道。看清楚，再出第一剑。'),
      turn('残响精灵·纱雾', 'guide', '先把一路上的三个选择分清。进入第二幕后签下的见证契约，决定由哪位同行者亲自证明一段事实；来到余烬登记库后选的修复章程，决定我们先修哪一段流程；眼前这座校场，只决定最后进灯塔前先带哪一种战场支援。三件事彼此独立，不需要选成同一个方向。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '见证契约选中的，是焰璃、澜音或鸦羽中的一人。她们能补上的不是抽象的“加成”，而是只有亲历者才说得清的证词：焰璃能说明封印怎样留下可检查的裂口，澜音能说明危险信号如何在正式警报前出现，鸦羽则能说明权限影线怎样被追踪和校准。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '但签过名字并不等于这段见证已经完成。那位同行者还要亲自取回能证明经历的信物，并从起源魔源前的共鸣会战中活着回来。只有她真正走完这条路，结案以后才可能留下自己的亲笔后日谈；否则档案不会替她编造一页不存在的证词。', { expression: 'focus' }),
      turn('绫星·璃', 'hero', '修复章程处理的是另一件事：我们究竟先把护送、校验还是接力恢复到可用状态。带回的护送印、校验簿或接力电容，会直接改变灯塔终战时我们能承受什么、又能先恢复哪一种工作。它和见证者是谁没有绑定关系。', { expression: 'resolve' }),
      turn('影织姬·鸦羽', 'shadow_boss', '还有，别把它和前面的契约、章程混在一起。那些已经写进记录了；这里选的只是临战备用。校验章程照样可以先拿护送，护送章程也能先拿校验。互不覆盖。'),
      turn('影织姬·鸦羽', 'shadow_boss', '先压倒决斗者，灯塔两场战斗都会少两轮追击；先解决咏唱者，守望者的魔法会变弱，勘误核心也失去二连击；先拿下总管，清场后共鸣储量会被重新补满。'),
      ...
      turn('影织姬·鸦羽', 'shadow_boss', '三名守卫最终都得停下，但只有最先击败的那一个会把备用术式装进我们的通行印。先看清它们各自怎么打、我们要付出什么，再决定第一剑指向谁。'),
      turn('残响精灵·纱雾', 'guide', '若你完成了接力章程，击败总管还会触发章程自己的那次回充。它和校场优先级是两份不同的准备。', { expression: 'focus' }),
      turn('绫星·璃', 'hero', '如果那位见证者已经取回信物，也活着走过了起源魔源前的会战，结案后她会留下只有自己能写的那一页。那不是打开结局的另一把钥匙，只是这一路共同经历最终在档案里留下了她本人的声音。', { expression: 'resolve' }),
      turn('绫星·璃', 'hero', '无论最后有没有多出那一页个人见证，只要我们打开灯塔并完成归档，灰港都会真正结案，未投递的信也都会重新上路。若某位同行者完整走完了自己的见证路线，她只是会在结案之后再留下属于自己的后记——不是更高一等的胜利，只是让她亲口说出这段经历后来变成了什么。', { expression: 'resolve' })
    ]),
    floor28: sequence('第二十八阵：归档风暴', [
      turn('旁白', null, '校场的三名守卫先后停止运转。最先落败的那一名守卫已把备用术式刻进灯塔通行印；另外两套程序随着校场停机，只留下不再发光的线路。这场选择已经做完，后面只需记住自己带走了哪份支援。', { kind: 'narration' }),
    ...
    floor30: sequence('第三十阵：余烬灯塔', [
      ...
      turn('旁白', null, '镜面下方有两具守卫机构。档案守望者站在外环，确保任何人不能在战斗中偷换原件；勘误核心藏在内环，一旦守望者停机，它就会立即开始清理所有与“无限救援”冲突的状态。两者是前后咬合的两道守关术式，中间不会留出休整时间。', { kind: 'narration' }),
      turn('档案守望者', 'act3_archive_warden', '我按旧协议保护原件。只要仍有一项矛盾，勘误核心就会把冲突内容全部抹掉，再照旧抄本重录；若整塔熄灭，它也会在复明时把“持续救援”重新写回来。', { expression: 'duty' }),
      turn('档案守望者', 'act3_archive_warden', '所以我不能因为你们带来了好消息就自行离岗。如果新卷在攻击中损坏，或有人趁我停机替换回执，勘误核心会在无法分辨真伪的情况下一口气抹掉所有冲突记录。我必须先亲自验证，你们能在我全力拦截时仍保住原件完整。', { expression: 'duty' }),
      ...
      turn('绫星·璃', 'hero', '这四步做完，登记网才会从“继续救援”改成“只留档、不再发令”。直接断电跳过任何一步，都会让它在复电时从旧副本重来，或留下没有投递状态的孤立原件。', { expression: 'resolve' }),
      turn('绫星·璃', 'hero', '我们之前做过的选择，现在都会在灯塔里兑现：章程决定哪一段修复先替我们承压，校场最先锁定的备用术式则决定临战支援。一路省下或花掉的卡牌、生命和魔力，不会在门口凭空重置。', { expression: 'resolve' }),
      turn('残响精灵·纱雾', 'guide', '三套修复章程不会因为到了最后一层就突然全部生效。我们在余烬登记库选定并真正修好的那一套，会一直保留到最后两战；接力校场最先锁定的备用术式则在它之外再提供一层支援。两份准备可以来自不同方向。开战前再确认一次守望者和核心的状态，别让一路做过的准备只停留在记忆里。', { expression: 'focus' }),
      ...
      turn('档案守望者', 'act3_archive_warden', '先让我验证你们能在攻击中守住原件。若我停机，勘误核心会立刻接管；它动手抹除前只有一次交接机会，中间不会休整。', { expression: 'duty' })
    ]),
    bossArchiveWardenPost: sequence('灯塔：守望者停机', [
      ...
      turn('档案守望者', 'act3_archive_warden', '原件验证通过，守望协议结束。勘误核心已接管；倒计时结束时，它仍会抹掉冲突记录，再照旧抄本重录。', { expression: 'duty' }),
      turn('档案守望者', 'act3_archive_warden', '我的长枪不会再指向你们。但我也无法代替你们填写新的判断：我只能证明进入写入口的东西与你们开战前带来的原件一致。接下来的每一个字，都必须由真正承担它的人填写。', { expression: 'duty' }),
      turn('旁白', null, '纱雾把光笔插进写入口：卷宗编号、来源、当前判断、经手人、修改时间、修改理由。旧文字全部封存，不再被后来的笔迹覆盖。', { cg: '/assets/anime/cg/liyue-traceable-revocation-cg-audit-v3.webp', cgHold: 9, kind: 'narration' }),
      ...
      turn('旁白', null, '勘误核心在内环完全醒来。它读到了撤销令，却仍按旧协议把这份“与无限救援相冲的新结论”列作最后一项错误。数百道红色校对线拢向写入口，准备在灯塔正式向外送出结案前将它抹掉。', { kind: 'narration' }),
      turn('绫星·璃', 'hero', '经手注记已经接上，撤销令已签，见证也在场。最后一步不是砸掉档案，而是击败还在执行旧抹除术式的勘误核心，护住这次这次有原件、有签名、也有清楚经手记录的新结论，让灯塔把新状态送到每一座哨站。', { expression: 'resolve' })
    ]),
    ending: sequence('终章：未投递的信', [
      ...
      turn('残响精灵·纱雾', 'guide', '如果第二幕签下见证契约的那位同行者取回了自己的信物，也从起源魔源前的会战中活着归来，她还会亲手留下一页自己的话。那一页不改变今天的胜负，只是不让那段同行被最后的结案省略。', { expression: 'gentle' }),
      turn('绫星·璃', 'hero', '所以结案本身已经完整：灰港的警报停止，名单和原件留下，信件重新开始投递，仍待核实的人也有了明确的复查人。若焰璃、澜音或鸦羽中的一人完成了自己的见证，她会在这些记录之后再添上一页亲笔留下的话。那一页不会推翻今天的结论，只会告诉后来的人，这份经历最后在她自己的生活里留下了什么。', { expression: 'resolve' }),
