...
function installDialogues(dialogues) {
  Object.assign(dialogues, {
    ...
    floor22: sequence('第二十二阵：夜航侧库', [
      ...
      turn('猫卫长·米露', 'cat_boss', '守柜人的攻击本身就是检验：如果连自己都护不住，护送印也不会相信我们能把信匣完整带出去。先看清他的攻击方式和自己还能承受多少，再决定要不要付出那两张月辉卡。这里考的是选择，不是服从。'),
    ...
    floor23: sequence('第二十三阵：逐页校验室', [
      ...
      turn('深蓝歌姬·澜音', 'whale_boss', '先别急着进内室。执行官的术式会直接穿过护甲，先确认我们吃得下那一轮魔法。赢下来的也不是一份“更好看的答案”，而是让互相矛盾的证据终于能在同一张桌上说得上话。'),
    ...
    floor24: sequence('第二十四阵：灯塔接力室', [
      ...
      turn('龙姬·焰璃', 'dragon_boss', '等我们在接力校场真正击败总管，电容还会完整回充一次。再往后就没有这样的补给了，那一轮魔力必须留给灯塔；这条路给的是续航，不会替我们直接削弱最后的敌人。'),
      turn('龙姬·焰璃', 'dragon_boss', '也别把“补满”听成从此可以挥霍。离开接力校场之后，不会再有章程替我们重新充能；档案守望者和勘误核心又会一前一后出现。这条路给的是一次明确的蓄能窗口，不是无限的魔力。'),
    ...
    floor27: sequence('第二十七阵：接力校场', [
      ...
      turn('影织姬·鸦羽', 'shadow_boss', '三名守卫最终都得停下，但只有最先击败的那一个会把备用程序装进我们的通行印。先看清它们各自怎么打、我们要付出什么，再决定第一剑指向谁。'),
    ...
    floor30: sequence('第三十阵：余烬灯塔', [
      ...
      turn('奥术主权者', 'arcane_sovereign', '别再往黑暗里找第四个人了。三日期限是我删的；三席那枚“同意”是核心拿旧回答补出来的；真正的离港回执，则被虚空先驱按我留下的错误命令截进了权限链。走到这里，我们已经把每一只手都看见了。', { expression: 'regret' }),
      ...
      turn('残响精灵·纱雾', 'guide', '先把灰港原卷锁成只读。求援、离港回执、名簿，还有我们一路查到的那些错误印记，全都原样留下。谁都不能为了让今天看起来更体面，就回头把三年前改成一份漂亮的故事。', { expression: 'focus' }),
      turn('残响精灵·纱雾', 'guide', '然后在原卷外面另开两栏：一栏只写现在知道的状态——已离港、已罹难、待核实、待投递；另一栏记谁根据什么证据改过它、什么时候改、为什么改。以后有新证据，就改状态，不碰旧原文。', { expression: 'focus' }),
      ...
      turn('绫星·璃', 'hero', '我们之前做过的选择，现在都会在灯塔里兑现：章程决定哪一段修复先替我们承压，校场最先锁定的备用程序则决定临战支援。一路省下或花掉的卡牌、生命和魔力，不会在门口凭空重置。', { expression: 'resolve' }),
      turn('残响精灵·纱雾', 'guide', '三套修复章程不会因为到了最后一层就突然全部生效。我们在余烬登记库选定并真正修好的那一套，会一直保留到最后两战；接力校场最先锁定的备用程序则在它之外再提供一层支援。两份准备可以来自不同方向。开战前再确认一次守望者和核心的状态，别让一路做过的准备只停留在记忆里。', { expression: 'focus' }),
    ...
    ending: sequence('终章：未投递的信', [
      ...
      turn('残响精灵·纱雾', 'guide', '如果第二幕签下见证契约的那位同行者取回了自己的信物，也从起源魔源前的会战中活着归来，她还会亲手留下一页后记。那一页不改变今天的胜负，只是不让那段同行被最后的结案省略。', { expression: 'gentle' }),
      turn('绫星·璃', 'hero', '所以结案本身已经完整：灰港的警报停止，名单和原件留下，信件重新开始投递，仍待核实的人也有了明确的复查人。若焰璃、澜音或鸦羽中的一人完成了自己的见证，她会在这些记录之后再添上一页亲笔后记。那一页不会推翻今天的结论，只会告诉后来的人，这份经历最后在她自己的生活里留下了什么。', { expression: 'resolve' }),
